...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `POST /api/trees/${treeId}/pictures called. req.files present?`,
      ...
      for (const file of files) {
        ...
        const newPath = path.join(
          treeDir,
          file.filename || file.originalname || `upload-${Date.now()}`
        );
        try {
          if (
            oldPath &&
            ...
            try {
              ...
              console.warn(
                "Failed to move uploaded file to public folder, continuing with available URL",
                e
              );
            }
          }
...
router.delete("/:treeId/pictures/:pictureId", async (req, res) => {
  try {
    ...
    // find picture
    const pic = await prisma.treePicture.findUnique({
      where: { id: pictureId },
    });
    if (!pic) return res.status(404).json({ error: "Picture not found" });
    if (pic.treeId !== treeId)
      return res
        .status(400)
        .json({ error: "Picture does not belong to this tree" });
    // delete DB record
