...
type Props = {
  ...
  setRoadsGeoJson: React.Dispatch<
...
const RoadEditor: React.FC<Props> = ({
  editingRoadId,
  editingProps,
  setEditingRoadId,
  setEditingProps,
      ...
      <PanelBox width="w-56 sm:w-64" className="p-2 sm:p-3">
        ...
        <div className="flex flex-col gap-2 text-xs sm:text-sm">
          ...
          <input
            ...
            onChange={(e) =>
              setEditingProps((s) => ({ ...s, nameroad: e.target.value }))
            }
          />
          ...
          <div className="grid grid-cols-5 gap-2">
            {PALETTE.map((sw) => (
              <button
                key={sw}
                type="button"
                onClick={() => setEditingProps((s) => ({ ...s, color: sw }))}
                title={sw}
          ...
          <select
            ...
            onChange={(e) =>
              setEditingProps((s) => ({ ...s, status: e.target.value }))
            }
          >
              ...
              onClick={async () => {
                try {
                  ...
                  try {
                    if (roadsGeoJson && Array.isArray(roadsGeoJson.features)) {
                      ...
                      updated.features = updated.features.map((f) => {
                        ...
                        if (
                          String(
                            (p as unknown as Record<string, unknown>).id || f.id
                  ...
                  if (typeof onError === "function")
                    onError(t("userMap.editRoad.failedMessage"));
