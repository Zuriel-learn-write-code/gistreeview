...
const AddTreeForm: React.FC<Props> = ({
  lat: passedLat,
  lng: passedLng,
  onCancel,
  onCreated,
  roadId: prefillRoadId,
  roadName: prefillRoadName,
  roadPickActive,
  onToggleRoadPick,
  editingTree,
  onError,
  ...
  const [trunkDiameter, setTrunkDiameter] = useState<number | undefined>(
    undefined
  );
  const [lbranchWidth, setLbranchWidth] = useState<number | undefined>(
    undefined
  );
  ...
  const [lat, setLat] = useState<number | undefined>(
    typeof passedLat === "number" ? passedLat : undefined
  );
  const [lng, setLng] = useState<number | undefined>(
    typeof passedLng === "number" ? passedLng : undefined
  );
  ...
  useEffect(() => {
    let mounted = true;
    const readers: Array<Promise<{ file: File; url?: string }>> = files.map(
      (f) => {
    ...
    Promise.all(readers)
      ...
      .catch(() => {
        if (mounted) setPreviews(files.map((f) => ({ file: f })));
      });
    return () => {
      mounted = false;
    };
  ...
  useEffect(() => {
    if (typeof prefillRoadId === "string" && prefillRoadId.trim())
      setRoadId(prefillRoadId);
    if (typeof prefillRoadName === "string" && prefillRoadName.trim())
      setRoadNameState(prefillRoadName);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof t.trunk_diameter !== "undefined")
        setTrunkDiameter(Number(t.trunk_diameter));
      if (typeof t.lbranch_width !== "undefined")
        setLbranchWidth(Number(t.lbranch_width));
      if (typeof t.status === "string")
        setStatus(t.status as "good" | "warning" | "danger");
      ...
      const relatedRoad = (
        t as unknown as { road?: { nameroad?: string } | null }
      ).road;
      if (relatedRoad && typeof relatedRoad.nameroad === "string")
        setRoadNameState(relatedRoad.nameroad);
  ...
  useEffect(() => {
    if (
      editingTree &&
      editingTree.treePictures &&
      Array.isArray(editingTree.treePictures)
    ) {
      const pics = editingTree.treePictures.map((p) => ({
        id: p.id,
        url: p.url,
      }));
  ...
  const submit = async () => {
    ...
    if (typeof lat !== "number" || typeof lng !== "number") {
      if (typeof onError === "function")
        onError(t("userMap.add.location_missing"));
    ...
    try {
      const payload: {
        ...
        age: age || undefined,
        trunk_diameter: trunkDiameter || undefined,
        lbranch_width: lbranchWidth || undefined,
        status,
      ...
      if (editingId) {
        ...
        const res = await fetch(apiUrl(`/api/trees/${editingId}`), {
          method: "PUT",
      ...
      if (files.length > 0 && createdId) {
        ...
        const up = await fetch(apiUrl(`/api/trees/${createdId}/pictures`), {
          method: "POST",
          body: fd,
        });
      ...
      if (typeof onError === "function")
        onError(t("userMap.add.create_failed"));
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <strong className="text-sm sm:text-base">
          {editingId ? t("userMap.add.edit") : t("userMap.add.title")}
        </strong>
        <button
      ...
      <div className="flex flex-col gap-2 text-xs sm:text-sm">
        <div>
          {roadNameState || roadId ? (
            <div className="text-xs text-gray-600 dark:text-gray-300">
              <div>
                {t("userMap.labels.road")}:{" "}
                <span className="font-semibold">{roadNameState || "-"}</span>
              </div>
        ...
        <div className="text-xs text-gray-500 dark:text-gray-300">
          {t("userMap.labels.coordinates")}:{" "}
          <span className="font-mono text-gray-700 dark:text-gray-300">
            {typeof lat === "number" ? lat.toFixed(6) : "-"}
          </span>{" "}
          {t("userMap.labels.lng")}:{" "}
          <span className="font-mono text-gray-700 dark:text-gray-300">
        ...
        <label
          htmlFor="addtree-species"
          className="text-xs text-gray-600 dark:text-gray-300"
        >
          {t("userMap.labels.species")}
        </label>
        <input
          id="addtree-species"
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <label
              htmlFor="addtree-trunk"
              className="text-xs text-gray-600 dark:text-gray-300"
            >
              {t("userMap.labels.trunk_diameter")}
            </label>
            <input
              id="addtree-trunk"
              type="number"
        ...
        <label
          htmlFor="addtree-lbranch"
          className="text-xs text-gray-600 dark:text-gray-300"
        >
          {t("userMap.labels.lbranch_width")}
        </label>
        <input
          id="addtree-lbranch"
          type="number"
        ...
        />
        <label
          htmlFor="addtree-status"
          className="text-xs text-gray-600 dark:text-gray-300"
        >
          {t("userMap.labels.status")}
        </label>
        <select
          id="addtree-status"
          className="border p-1 rounded"
          value={status}
          onChange={(e) =>
            setStatus(e.target.value as "good" | "warning" | "danger")
          }
        >
        ...
        <label
          htmlFor="addtree-ownership"
          className="text-xs text-gray-600 dark:text-gray-300"
        >
          {t("userMap.labels.ownership")}
        </label>
        <input
          id="addtree-ownership"
          className="border p-1 rounded"
          value={ownership}
        ...
        <div className="flex gap-2 items-center">
          <input
            id="addtree-roadname"
            className="border p-1 rounded w-full bg-gray-50 dark:bg-gray-800 text-sm"
            value={roadNameState || roadId || ""}
            readOnly
            aria-readonly="true"
          />
          <button
            ...
            onClick={() => {
              try {
                console.log("[AddTreeForm] Select road clicked", {
                  current: roadPickActive,
                  next: !roadPickActive,
                });
              } catch {
                /* ignore */
              ...
              try {
                console.log(
                  "[AddTreeForm] onToggleRoadPick typeof",
                  typeof onToggleRoadPick
                );
              } catch {
                /* ignore */
              }
              if (typeof onToggleRoadPick === "function")
                onToggleRoadPick(!roadPickActive);
            }}
        ...
        <label
          htmlFor="addtree-description"
          className="text-xs text-gray-600 dark:text-gray-300"
        >
          {t("userMap.labels.description")}
        </label>
        <textarea
          id="addtree-description"
          className="border p-1 rounded"
          value={description}
        ...
        <div className="flex flex-col gap-2">
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const res = await fetch(
                            apiUrl(`/api/trees/${editingId}/pictures/${p.id}`),
                            { method: "DELETE" }
                          );
                          if (!res.ok) throw new Error("delete failed");
                          setExistingPictures((cur) =>
                            cur.filter((x) => x.id !== p.id)
                          );
                        } catch (err) {
                          console.error("Failed to delete picture", err);
                          if (typeof onError === "function")
                            onError("Failed to delete picture");
                          else alert("Failed to delete picture");
                        }
                      }}
          ...
          <label className="inline-flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1 bg-gray-100 border rounded flex items-center gap-2"
            >
              <img
                src="/icons/upload-svgrepo-com.svg"
                alt="upload"
                className="w-5 h-5"
              />
            </button>
            <span className="text-xs text-gray-500 dark:text-gray-300">
              {t("userMap.add.upload_hint")}
            </span>
          </label>
              ...
              {files.map((f, idx) => (
                <div key={`${f.name}-${idx}`} className="w-24">
                  {previews[idx] && previews[idx].url ? (
                    ...
                    <div className="w-24 h-16 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded text-xs text-gray-600 dark:text-gray-300">
                      {f.name}
                    </div>
                  )}
                  <div className="flex items-center justify-between text-xs mt-1">
