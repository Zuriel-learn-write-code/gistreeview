...
const AMBON_BOUNDS: [[number, number], [number, number]] = [
  [-3.95, 128.03],
  [-3.5, 128.35],
];
...
const UserMaps: React.FC<UserMapsProps> = ({
  height = "600px",
  onMapReady,
  onTreeClick,
}) => {
  ...
  const [role, setRole] = useState<"admin" | "officer" | "user" | null>(() =>
    getUserRole()
  );
  ...
  useEffect(() => {
    ...
    const onStorage = (ev: StorageEvent) => {
      try {
        if (ev.key === "user") setRole(getUserRole());
      } catch {
        /* ignore */
      }
    };
    try {
      window.addEventListener(
        "gistreeview:user-logged-out",
        onLoggedOut as EventListener
      );
    } catch {
      /* ignore */
    }
  ...
  const [baseLayer, setBaseLayer] = useState<
    "osm" | "carto" | "transport" | "topo" | "dark"
  >(defaultBase);
  // preserve prior non-dark layer so we can restore when dark mode is turned off
  const prevBaseLayerRef = useRef<
    "osm" | "carto" | "transport" | "topo" | null
  >(null);
  ...
  const setExclusiveControl = useCallback(
    (name: string | null) => {
      ...
      setActiveControl((prevActive) => {
        ...
        try {
          console.debug("[UserMaps] setExclusiveControl ->", {
            requested: name,
            newActive,
            turningOn,
            prevActive,
        ...
        if (newActive === "editRoad") {
          ...
          try {
            (
              window as unknown as { __gistreeviewEditMode?: boolean }
            ).__gistreeviewEditMode = true;
          } catch {
            /* ignore */
          }
        } else {
        ...
        if (newActive !== "locate") {
          try {
            ...
            const container = (
              mapInstance as unknown as { getContainer?: () => HTMLElement }
            ).getContainer?.();
            if (container) container.style.cursor = "";
          } catch (err) {
            console.debug("UserMaps: ignored error", err);
          }
        }
        ...
        try {
          (
            window as unknown as { __gistreeviewEditMode?: boolean }
          ).__gistreeviewEditMode = newActive === "editRoad";
        } catch {
          /* ignore */
        }
        try {
          console.debug("[UserMaps] set __gistreeviewEditMode ->", {
            value: newActive === "editRoad",
          });
        } catch {
          /* ignore */
        }
  ...
  const setExclusiveControlRef = useRef<typeof setExclusiveControl | null>(
    null
  );
  useEffect(() => {
    setExclusiveControlRef.current = setExclusiveControl;
  }, [setExclusiveControl]);
  // coordinate picker box when locateMode is active
  const [coordBoxLatLng, setCoordBoxLatLng] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
  ...
  useEffect(() => {
    try {
      console.log("[UserMaps] roadPickMode changed", { roadPickMode });
    } catch {
      /* ignore */
    }
  }, [roadPickMode]);
  ...
  const setRoadPickModeSafe = (v: boolean) => {
    try {
      (
        window as unknown as { __gistreeviewRoadPick?: boolean }
      ).__gistreeviewRoadPick = v;
    } catch {
      /* ignore */
    }
    setRoadPickMode(v);
  ...
  useEffect(() => {
    try {
      if (editingTree && editTreeMode) {
        console.debug(
          "[UserMaps] effect: editingTree & editTreeMode true -> showEditForm",
          { editingTreeId: editingTree?.id, editTreeMode, showEditForm }
        );
        setShowEditForm(true);
      }
    } catch {
  ...
  useEffect(() => {
    try {
      if (editingRoadId && activeControl === "editRoad") {
        console.debug(
          "[UserMaps] effect: road selected for edit -> showEditForm",
  ...
  const TF_KEY =
    typeof (
      import.meta as unknown as { env?: { VITE_THUNDERFOREST_KEY?: string } }
    )?.env?.VITE_THUNDERFOREST_KEY === "string"
      ? (import.meta as unknown as { env: { VITE_THUNDERFOREST_KEY: string } })
          .env.VITE_THUNDERFOREST_KEY
      : "";
  const transportUrl = TF_KEY
    ? `https://{s}.tile.thunderforest.com/transport/{z}/{x}/{y}.png?apikey=${TF_KEY}`
  ...
  useEffect(() => {
    try {
      ...
      console.debug("UserMaps detect dark-on-mount check failed", err);
    }
    // run once on mount
    ...
    try {
      ...
      const onChange = (ev: MediaQueryListEvent) => {
        try {
          ...
          console.debug("UserMaps.matchMedia change handler failed", err);
        }
      };
      // use onchange property which is widely supported instead of addEventListener/removeEventListener
      try {
        (mq as MediaQueryList).onchange = onChange;
      } catch (err) {
        console.debug("UserMaps: failed to set mq.onchange", err);
      }
      return () => {
        try {
          (mq as MediaQueryList).onchange = null;
        } catch (err) {
          console.debug("UserMaps: failed to clear mq.onchange", err);
        }
      };
    } catch (err) {
      console.debug("UserMaps: ignored error", err);
    ...
    try {
      ...
      const mo = new MutationObserver(() => {
        try {
          ...
          console.debug("UserMaps.MutationObserver handler failed", err);
        }
      });
      ...
      return () => {
        try {
          ...
          console.debug("UserMaps: MutationObserver.disconnect failed", err);
        }
      };
    } catch (err) {
      console.debug("UserMaps: ignored error", err);
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (r.ok) {
          try {
              ...
              data.features = data.features.map((f: GeoJSON.Feature) => {
                ...
                try {
                  if (
                    ...
                    props.color.startsWith("palette_")
                  ) {
                    const hex =
                      PALETTE_ENUM && PALETTE_ENUM[props.color]
                        ? PALETTE_ENUM[props.color]
                        : null;
            ...
            console.warn(
              "Failed to parse /api/roads/with-treecount response, falling back",
              e
            );
          }
        }
            ...
            try {
              ...
              if (raw) {
                ...
                if (s.startsWith("palette_")) {
                  colorHex =
                    PALETTE_ENUM && PALETTE_ENUM[s] ? PALETTE_ENUM[s] : null;
            ...
            return {
              ...
              ...({
                ...
                properties: {
                  ...
                  treesCount:
                    typeof row.treesCount === "number" ? row.treesCount : 0,
                },
                geometry,
        ...
        setEditMessage(
          "Cannot load roads from backend API — editing disabled."
        );
      } finally {
    ...
    const zoomIn = document.querySelector<HTMLElement>(
      ".leaflet-control-zoom a.leaflet-control-zoom-in"
    );
    const zoomBtn =
      zoomIn || document.querySelector<HTMLElement>(".leaflet-control-zoom a");
    if (zoomBtn) {
      ...
      try {
        ...
        if (btnEl && cs) {
          ...
          const increasedWidth =
            parsedWidth > 0 ? `${Math.max(2, parsedWidth + 2)}px` : "2px";
    ...
    try {
      ...
      if (locateMode) {
        ...
        container.style.cursor =
          "url('/icons/pin-svgrepo-com.svg') 18 36, auto";
      } else {
        container.style.cursor = "";
      }
      return () => {
  ...
  useEffect(() => {
    ...
    if (locateMode || addTreeMode || pickedLatLng)
  ...
  const [popupPos, setPopupPos] = useState<{ lat: number; lng: number } | null>(
    null
  );
  ...
  const handleShowPopup = (
    ...
    try {
      if (pos && mapInstance) {
        const m = mapInstance as unknown as LeafletMap & {
          panTo?: (p: [number, number]) => void;
          setView?: (p: [number, number]) => void;
        };
  ...
  useEffect(() => {
    ...
    try {
      ...
      setEditingProps((s) => ({
        ...s,
        nameroad: name,
        color: s.color || p.color || "#ff5722",
        status: s.status || (p.status as string) || "unknown",
      }));
    } catch (e) {
      void e;
    }
  ...
  useEffect(() => {
    ...
    try {
      const layer = L.geoJSON(
        roadsGeoJson as unknown as GeoJSON.FeatureCollection
      );
      const bounds = layer.getBounds();
      const isValidBounds =
        typeof (bounds as unknown as { isValid?: () => boolean }).isValid ===
        "function"
          ? (bounds as unknown as { isValid: () => boolean }).isValid()
          : Boolean(bounds);
      const typedMap = mapInstance as unknown as LeafletMap & {
        fitBounds?: (
          b: L.LatLngBoundsExpression,
          o?: L.FitBoundsOptions
        ) => void;
      };
      if (isValidBounds && typeof typedMap.fitBounds === "function") {
        try {
          typedMap.fitBounds(bounds, { padding: [40, 40] });
        } catch (e) {
          void e;
        }
  ...
  useEffect(() => {
    ...
    const onBase = (e: unknown) => {
      console.log("baselayerchange", e);
    };
    const onOverlayAdd = (e: unknown) => {
    ...
    if (ctrl && ctrl instanceof HTMLElement) {
  ...
  useEffect(() => {
    try {
      console.debug(
        "[UserMaps] diagnostic: editingRoadId/showEditForm changed",
  ...
  const [filters, setFilters] = useState<{
    ...
    trees: { good: boolean; warning: boolean; danger: boolean };
  }>({
  ...
  useEffect(() => {
    try {
      console.log(
        "[UserMaps] mounting: installing road-selected-for-edit listener"
      );
    } catch {
      /* ignore */
    }
    const onSelectForEdit = (ev: Event) => {
      try {
        ...
        try {
          console.log(
            "[UserMaps] received gistreeview:road-selected-for-edit",
            {
              detail,
              editModeRefCurrent: editModeRef.current,
              activeControl: activeControlRef.current,
            }
          );
        } catch {
          /* ignore */
    ...
    try {
      window.addEventListener(
        "gistreeview:road-selected-for-edit",
        onSelectForEdit as EventListener
      );
    } catch {
      /* ignore */
    }
    ...
    try {
      (
        window as unknown as { __gistreeview_userMaps_listeners?: boolean }
      ).__gistreeview_userMaps_listeners = true;
    } catch {
      /* ignore */
    }
    try {
      console.log(
        "[UserMaps] installed gistreeview:road-selected-for-edit listeners (window+capture)"
      );
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    try {
      (window as unknown as { showAlert?: typeof showAlert }).showAlert =
        showAlert;
    } catch {
      /* ignore */
    }
    return () => {
      try {
        delete (window as unknown as { showAlert?: typeof showAlert })
          .showAlert;
      } catch {
        /* ignore */
      }
  ...
  useEffect(() => {
    ...
    try {
      window.addEventListener("gistreeview:close-popup", onClosePopup);
    } catch {
      /* ignore */
    }
    return () => {
      try {
        window.removeEventListener("gistreeview:close-popup", onClosePopup);
      } catch {
        /* ignore */
      }
    };
  ...
  useEffect(() => {
    try {
      console.log(
        "[UserMaps] mounting: installing gistreeview:road-click listener"
      );
    } catch {
      /* ignore */
    }
    ...
    const onRoadClick = (ev: Event) => {
      try {
        ...
        const id = detail.id ?? detail.uuid ?? detail.id;
        try {
          console.log("[UserMaps] received gistreeview:road-click", {
            detail,
            editModeRefCurrent: editModeRef.current,
            activeControlRefCurrent: activeControlRef.current,
          });
        } catch {
          /* ignore */
        }
        ...
        if (shouldHandle) {
          try {
            if (ev && typeof (ev as CustomEvent).preventDefault === "function")
          ...
          const status = detail.status as string | undefined;
          try {
            console.log("[UserMaps] handling road-click -> opening editor", {
              sid,
              name,
              activeControlRefCurrent: activeControlRef.current,
            });
    ...
    try {
      window.addEventListener(
        "gistreeview:road-click",
        onRoadClick as EventListener
      );
    } catch {
      /* ignore */
    }
    ...
    try {
      console.log(
        "[UserMaps] installed gistreeview:road-click listeners (window+capture)"
      );
    } catch {
      /* ignore */
    }
  ...
  const getHorrorIconForZoom = useCallback((zoom: number) => {
    ...
    const t = Math.min(
      1,
      Math.max(0, (zoom - minVisibleZoom) / (maxZoom - minVisibleZoom))
    );
    const size = Math.round(18 + (64 - 18) * t);
    if (horrorIconCacheRef.current[size])
      return horrorIconCacheRef.current[size];
    const icon = L.icon({
      iconUrl: "/icons/tree-horror-svgrepo-com.svg",
      iconSize: [size, size],
      iconAnchor: [Math.round(size / 2), size],
      popupAnchor: [0, -Math.round(size / 2)],
      className: "tree-horror-icon",
    });
    horrorIconCacheRef.current[size] = icon;
    return icon;
  }, []);

  const getIconForZoom = useCallback(
    (status: "good" | "warning" | "danger", zoom: number) => {
      ...
      if (useSvg) {
        ...
        const svg = `<?xml version="1.0" encoding="UTF-8"?><svg xmlns='http://www.w3.org/2000/svg' width='${size}' height='${size}' viewBox='0 0 ${size} ${size}'><circle cx='${
          size / 2
        }' cy='${size / 2}' r='${r}' fill='${colorForStatus}'/></svg>`;
  ...
  const geoStyle = useCallback(
    (feature: GeoJSON.Feature | undefined) => {
      ...
      const highlight =
        editingRoadId === props.id ? Math.max(weight, 6) : weight;
      return {
  ...
  useEffect(() => {
    try {
      (
        window as unknown as { __gistreeviewEditMode?: boolean }
      ).__gistreeviewEditMode = editMode;
      ...
      <div ref={wrapperRef} className="relative w-full" style={{ height }}>
        ...
        {activeControl ? (
          <div className="absolute left-1/2 transform -translate-x-1/2 top-3 z-[10002] hidden sm:flex">
            <div
              className="bg-white/95 dark:bg-gray-800/90 text-gray-900 dark:text-gray-100 px-3 py-1 rounded-full shadow flex items-center gap-3"
              style={{ minWidth: 220 }}
            >
              <div className="flex items-center gap-3">
                <div className="text-sm font-semibold">
                  {activeControl === "layers"
                    ? t("userMap.activeControls.layers")
                    : activeControl === "locate"
                    ? t("userMap.activeControls.locate")
                    : activeControl === "editRoad"
                    ? t("userMap.activeControls.editRoad")
                    : activeControl === "filters"
                    ? t("userMap.activeControls.filters")
                    : activeControl === "addTree"
                    ? t("userMap.activeControls.addTree")
                    : activeControl === "editTree"
                    ? t("userMap.activeControls.editTree")
                    : activeControl === "deleteTree"
                    ? t("userMap.activeControls.deleteTree")
                    : ""}
                </div>
                <div className="text-xs text-gray-600 dark:text-gray-300">
                  ...
                  {activeControl === "deleteTree" &&
                    t("userMap.help.deleteTree")}
                </div>
              </div>
              <button
                className="ml-auto text-sm text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-white px-1"
                onClick={() => activateControl(null)}
                aria-label="Close active control"
              >
                ×
              </button>
            </div>
        ...
        <div
          className="flex flex-col gap-2 absolute left-3"
          style={{ top: 10, zIndex: 10000 }}
        >
            ...
            <>
              ...
              <div className="mt-2">
                <LayerPicker
                  ...
                  onChange={(v) => {
                    setBaseLayer(v);
                    showAlert("info", "Layer switched", `Now showing ${v}`);
                  }}
                  btnSize={btnSize}
                  open={activeControl === "layers"}
                  onOpenChange={(v) => setExclusiveControl(v ? "layers" : null)}
                />
              </div>
              ...
              <div className="mt-2">
                ...
                {locateMode && coordBoxLatLng ? (
                  <div
                    ...
                    style={{ zIndex: 10001, top: 0 }}
                  >
                    <PanelBox width="w-56 sm:w-64" className="p-2 sm:p-3">
                      <div className="flex items-center justify-between mb-2">
                        <strong className="text-gray-900 dark:text-gray-100 text-sm sm:text-base">
                          {t("userMap.coordinates.title")}
                        </strong>
                        <button
                      ...
                      <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 mb-2">
                        {t("userMap.coordinates.lat")}:{" "}
                        <span className="font-mono text-gray-800 dark:text-gray-200">
                          {coordBoxLatLng.lat.toFixed(6)}
                        </span>
                      </div>
                      <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 mb-2">
                        {t("userMap.coordinates.lng")}:{" "}
                        <span className="font-mono text-gray-800 dark:text-gray-200">
                          {coordBoxLatLng.lng.toFixed(6)}
                        </span>
                      </div>
                      <div className="flex gap-2">
                        <button
                          className="px-2 py-1 text-xs sm:px-3 sm:py-1 bg-blue-500 text-white rounded"
                          onClick={() => {
                            try {
                              const txt = `${coordBoxLatLng.lat},${coordBoxLatLng.lng}`;
                              void navigator.clipboard.writeText(txt);
                              showAlert(
                                "success",
                                t("userMap.coordinates.copiedTitle"),
                                t("userMap.coordinates.copiedMessage")
                              );
                            } catch (e) {
                              void e;
                              showAlert(
                                "error",
                                t("userMap.coordinates.copyFailedTitle"),
                                t("userMap.coordinates.copyFailedMessage")
                              );
                            }
                          }}
                        >
                        ...
                        <button
                          className="px-2 py-1 text-xs sm:px-3 sm:py-1 bg-gray-200 dark:bg-gray-600 dark:text-gray-100 rounded"
                          onClick={() => {
                            setCoordBoxLatLng(null);
                          }}
                        >
                ...
                <div className="mt-2">
                  <div className="flex items-center gap-2">
                    <EditRoadButton
                      ...
                      active={activeControl === "editRoad" || editMode}
                      btnSize={btnSize}
                    />
                      ...
                      <PanelBox width="w-56 sm:w-64" className="p-2 sm:p-3">
                        ...
                        <div className="flex flex-col gap-2 text-xs sm:text-sm">
                          <div className="text-xs text-gray-700 dark:text-gray-300">
                            {t("userMap.editRoad.idLabel")}
                            <span
                              title={editingRoadId || undefined}
                              className="font-mono text-gray-800 dark:text-gray-300"
                            >
                              <span className="inline sm:hidden text-xs">
                                {shortenId(editingRoadId)}
                              </span>
                          ...
                          <label
                            htmlFor="user-editroad-name"
                            className="text-xs text-gray-600 dark:text-gray-300"
                          >
                            {t("userMap.editRoad.nameLabel")}
                          </label>
                          <input
                            id="user-editroad-name"
                            className="border p-1 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-xs sm:text-sm"
                            value={editingProps.nameroad || ""}
                            onChange={(e) =>
                              setEditingProps((s) => ({
                                ...s,
                                nameroad: e.target.value,
                              }))
                            }
                          />
                          <label className="text-xs text-gray-600 dark:text-gray-300">
                            {t("userMap.editRoad.colorLabel")}
                          </label>
                          <div className="grid grid-cols-5 gap-2">
                            {PALETTE.map((sw) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  setEditingProps((s) => ({ ...s, color: sw }))
                                }
                                title={sw}
                                aria-label={`Select color ${sw}`}
                                className={`w-6 h-6 rounded border ${
                                  editingProps.color === sw
                          ...
                          <label
                            htmlFor="user-editroad-status"
                            className="text-xs text-gray-600 mt-2 dark:text-gray-300"
                          >
                            {t("userMap.editRoad.statusLabel")}
                          </label>
                          <select
                            id="user-editroad-status"
                            className="border p-1 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-xs sm:text-sm"
                            value={editingProps.status || "unknown"}
                            onChange={(e) =>
                              setEditingProps((s) => ({
                                ...s,
                          ...
                          >
                            <option value="primary">
                              {t("userMap.editRoad.statusOptions.primary")}
                            </option>
                            <option value="secondary">
                              {t("userMap.editRoad.statusOptions.secondary")}
                            </option>
                            <option value="tertiary">
                              {t("userMap.editRoad.statusOptions.tertiary")}
                            </option>
                            <option value="unknown">
                              {t("userMap.editRoad.statusOptions.unknown")}
                            </option>
                          </select>
                              ...
                              onClick={async () => {
                                try {
                                  const body: {
                                    ...
                                    color?: string;
                                    status?: string;
                                  } = { nameroad: editingProps.nameroad };
                                  if (editingProps.color)
                                    body.color = editingProps.color;
                                  if (editingProps.status)
                                    body.status = editingProps.status;
                                  const res = await fetch(
                                    apiUrl(`/api/roads/${editingRoadId}`),
                                    {
                                      method: "PUT",
                                      headers: {
                                  ...
                                  try {
                                    if (
                                      ...
                                      updated.features = updated.features.map(
                                        (f) => {
                                  ...
                                  showAlert(
                                    "success",
                                    t("userMap.editRoad.savedTitle"),
                                    t("userMap.editRoad.savedMessage")
                                  );
                                } catch (err) {
                                  ...
                                  showAlert(
                                    "error",
                                    t("userMap.editRoad.failedTitle"),
                                    t("userMap.editRoad.failedMessage")
                                  );
                                }
                              }}
                              type="button"
              ...
              <div className="mt-2">
                <StatusFilterButton
                  ...
                  btnSize={btnSize}
                  open={activeControl === "filters"}
                  onOpenChange={(v) =>
                    setExclusiveControl(v ? "filters" : null)
                  }
                />
              </div>
        ...
        <div
          ...
          style={{ top: 10, zIndex: 10000, overflow: "visible" }}
        >
          <div className="flex flex-col gap-2">
            ...
            {role === "admin" && (
              <AddTreeButton
                ...
                onCreated={() => {
                  ...
                  showAlert(
                    "success",
                    t("userMap.tree.addedTitle"),
                    t("userMap.tree.addedMessage")
                  );
                }}
                ...
                onToggleRoadPick={(v) => {
                  try {
                    console.log("[UserMaps] AddTree onToggleRoadPick called", {
                      requested: v,
                    });
                  } catch {
                    /* ignore */
            ...
            {(role === "officer" || role === "admin") && (
              <EditTreeButton
                ...
                onToggle={() => {
                  ...
                  setExclusiveControl(
                    activeControl !== "editTree" ? "editTree" : null
                  );
                }}
                btnSize={btnSize}
                ...
                editingTree={editingTree}
                onCancel={() => {
                  /* keep edit-tree control active when user closes panel */ setShowEditForm(
                    false
                  );
                  setEditingTree(null);
                  setRoadPickModeSafe(false);
                ...
                onCreated={() => {
                  ...
                  showAlert(
                    "success",
                    t("userMap.tree.updatedTitle"),
                    t("userMap.tree.updatedMessage")
                  );
                }}
                roadId={pickedRoadId || undefined}
                roadName={pickedRoadName || undefined}
            ...
            {role === "admin" && (
              <DeleteTreeButton
                ...
                setTrees={setTrees}
                onDeleted={(count: number) => {
                  showAlert(
                    "success",
                    t("userMap.tree.deletedTitle"),
                    t("userMap.tree.deletedMessage", { count })
                  );
                }}
              />
            )}
        ...
        <MapContainer
          ...
          ref={(mapRef) => {
            if (mapRef && typeof (mapRef as unknown) !== "function") {
              try {
                setMapInstance(mapRef as unknown as LeafletMap);
              } catch (e) {
                void e;
              }
            }
        ...
        >
          ...
          {baseLayer === "dark" && (
            <TileLayer
              ...
              maxNativeZoom={22}
              maxZoom={22}
              attribution='&copy; <a href="https://carto.com/attributions">CARTO Dark</a>'
            />
          )}
          {baseLayer === "transport" && (
            <TileLayer
              ...
              maxNativeZoom={22}
              maxZoom={22}
              attribution="Map data &copy; OpenStreetMap contributors, Tiles &copy; Thunderforest (or HOT fallback)"
            />
          )}
          {baseLayer === "topo" && (
            <TileLayer
          ...
          {Array.isArray(trees)
            ? trees
                .filter((t) => {
                  ...
                  return Boolean(
                    (filters.trees as Record<string, boolean>)[st]
                  );
                })
                .map((t) => {
                  return (
          ...
          {popupPos && popupContent ? (
            ...
            >
              <MapPopup
                map={mapInstance}
                position={popupPos}
                onClose={() => handleShowPopup(null, null)}
              >
                {popupContent}
              </MapPopup>
            </div>
